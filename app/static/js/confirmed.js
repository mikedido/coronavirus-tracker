//initialisation
const width = document.getElementById("container").offsetWidth * 0.50,
height = 500,
legendCellSize = 30,
...
function addTooltip() {
    ...
    tooltip.append("polyline") // The rectangle containing the text, it is 210px width and 60 height
        .attr("points","0,0 210,0 210,65 0,65 0,0")
        .style("fill", "#222b1d")
        .style("stroke","black")
        .style("opacity","0.9")
...
Promise.all(promises).then(function(values) {
    ...
    var b  = path.bounds(geojson),
        ...
        const min = d3.min(scores['data'], function(e) { return +e.latest; }),
        max = d3.max(scores['data'], function(e) { return +e.latest; });
        ...
        //addTitle(scores['last_updated'], scores['latest']);
        
        scores['data'].forEach(function(element, i) {
            var countryPath = d3.select("#code" + element.country_code);
            
            countryPath
                .attr("scorecolor", quantile(+element.latest))
                .style("fill", function(d) { return quantile(+element.latest); })
                .on("mouseover", function(d) {
                ...
                .on("mouseout", function(d) {
                    countryPath.style("fill", function(d) { return quantile(+element.latest); });
